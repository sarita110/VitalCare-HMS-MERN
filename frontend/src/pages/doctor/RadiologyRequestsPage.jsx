// pages/doctor/RadiologyRequestsPage.jsx
...
const RadiologyRequestsPage = () => {
  ...
  const fetchRadiologyTests = useCallback(async (page = 1, currentFilters) => {
    setIsLoading(true);
    setError(null);
    try {
      const params = {
      ...
      const response = await doctorService.getDoctorRadiologyResults(params);
      if (response.success) {
      ...
      console.error("Fetch radiology tests error:", err);
      setError(err.message || "Could not load radiology tests.");
      toast.error(err.message || "Could not load radiology tests.");
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    fetchRadiologyTests(currentPage, filters);
  }, [fetchRadiologyTests, currentPage, filters]);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: "findings",
        Cell: ({ value, row }) =>
          row.original.status === "completed" && value ? (
            <Link
              to={`/doctor/radiology-results/${row.original._id}`}
