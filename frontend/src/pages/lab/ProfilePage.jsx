// src/pages/lab/ProfilePage.jsx
import React, { useContext } from "react";
import { Link } from "react-router-dom";
import LabContext from "../../context/LabContext"; // Use LabContext
...
const ProfilePage = () => {
  ...
  // LabContext fetches detailed profile including staff details if available
  const { labProfile, loadingProfile, profileError } = useContext(LabContext);

  if (loadingProfile && !labProfile) {
  ...
  const displayUser = labProfile?.user || authUser;
  const displayStaffDetails = labProfile?.staffDetails || {}; // Staff details (employeeId, qualifications etc)
  const displayHospital = labProfile?.hospital || {};
  const displayDepartment = displayUser?.department || {}; // Department from User object
      ...
      <Card>
        <div className="flex flex-col md:flex-row items-center md:items-start space-y-4 md:space-y-0 md:space-x-6">
          ...
          <div className="flex-grow text-center md:text-left">
            ...
            <p className="text-md text-primary-700">Lab Technician</p>
